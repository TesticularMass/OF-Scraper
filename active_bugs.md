...
- **#54 Additional circular imports beyond #52 (CLOSED 2026-05-02)** — Multiple cycles in the codebase resolved by deferring eager module-top calls + import edges to function-local. Chain edges removed:
  1. `managers/manager.py:11` was `from ofscraper.commands.db import db` (sole use at line 142). Deferred to function-local at the call site.
  2. `managers/manager.py:11-15` was 4 module-top command imports (`metadata`, `scraper`, `manual`, `check`). All four moved into the matching branch of `pick()` dispatch.
  3. `utils/live/screens.py:59` was `"layout": get_download_group()` — eagerly called factory at module-top SCREENS dict init. Changed to factory references (`get_download_group` and `lambda: <group>` for the rest); `setup_live` invokes `config["layout"]()` lazily.
  4. `scripts/naming_script.py:11-12` was `from ofscraper.classes.of.{posts,media}` at module top. Moved inside `naming_script()` function (only consumers via `isinstance` checks).
  5. `managers/postcollection.py:5-6` was `from ofscraper.classes.of.{posts,media}` at module top. Switched to `from __future__ import annotations` + `TYPE_CHECKING` for type hints; runtime constructor/`isinstance` calls moved into `_process_and_add_post`.
  6. `classes/of/media.py:16` was `import ofscraper.managers.manager as manager` (sole use at line 470 in `parse_mpd`). Deferred to function-local.
  7. `db/operations_/wrapper.py:10` was `import ofscraper.classes.placeholder as placeholder` (used inside the two decorator `inner` functions). Deferred to two function-local imports.
  8. `data/posts/scrape_paid.py:5` was `import ofscraper.commands.metadata.metadata as metadata` (sole use at line 121). Deferred to function-local.
  Verified post-fix: `import ofscraper`, `python -m ofscraper --help`, `from ofscraper.classes.of import posts, models, media`, `from ofscraper.commands import check, manual`, `from ofscraper.commands.metadata import metadata`, `from ofscraper.commands.scraper import scraper`, `from ofscraper.data.api import archive, messages, timeline, pinned, highlights, labels`, `from ofscraper.data.posts import scrape_paid, post`, `from ofscraper.db.operations_ import media, posts`, `from ofscraper.classes.placeholder import basePlaceholder`, `from ofscraper.classes.table.fields import selectfield, responsefield`, `from ofscraper.utils.live import screens, groups` — all import cleanly. CLOSED.
- **#53 `DownloadField.compare` (classes/table/fields/downloadfield.py:60-65) drops `str()` cast** — sibling override of `SelectField.compare` which Batch 1 hardened to `str(value) in self.query_one(SelectionList).selected` for #17. Override at line 60 reverts to bare `value in ...`. Currently safe because both Selection values (`"normal"`/`"protected"`/`"n/a"`) and the column row data are strings, but if a future column wires bool-typed data through `DownloadField` the bug class re-emerges. Defensive `str(value)` would harden in line with the sibling.
