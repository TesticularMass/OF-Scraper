...
- **#54 Additional circular imports beyond #52 (HIGH, NOT FIXED)** — Two more cycles surfaced after the #52 partial fix: (a) `commands/check.py` → `data/api/archive.py` → `db/operations_/media.py` → `db/operations_/wrapper.py` → `classes/placeholder.py` → `managers/manager.py` → `commands/db.py` → re-imports `db/operations_/media.py` mid-init. (b) `classes/of/posts.py` → `classes/of/media.py` → `managers/manager.py` → `managers/model.py` → `data/models/utils/retriver.py` → `data/api/subscriptions/lists.py` → `utils/live/screens.py` → eagerly calls `get_download_group()` at module top → reaches `utils/settings.setup_settings` → `config_file.open_config` → `prompts.prompt_validators` → `classes/placeholder.py` → `scripts/naming_script` → re-imports `classes/of/posts.py` mid-init. Each cycle has multiple eager module-top calls that need to be deferred (e.g. `screens.py:59` `"layout": get_download_group()` should be lazy). Architectural cleanup, not a one-line fix.
