...
- **#54 Additional circular imports beyond #52 (HIGH, PARTIAL — multiple cycles remain)** — At least three independent cycles in the codebase. (a) `commands/check.py` → `archive.py` → `db/operations_/media.py` → `wrapper.py` → `placeholder.py` → `managers/manager.py` → was `commands/db.py` → re-import `db/operations_/media`. **One edge removed 2026-05-02:** `manager.py:11 from ofscraper.commands.db import db` deferred to function-local at `manager.py:142` (only call site). Further down the chain `messages.py:25` re-imports `db/operations_/media` mid-init, so `from ofscraper.commands import check` still fails. (b) `classes/of/posts.py` → `classes/of/media.py` → `managers/manager.py` → `managers/model.py` → `data/models/utils/retriver.py` → `data/api/subscriptions/lists.py` → `utils/live/screens.py` → eagerly calls `get_download_group()` at module top → walks settings init → `prompts.prompt_validators` → `classes/placeholder.py` → `scripts/naming_script` → re-imports `classes/of/posts.py` mid-init. Untouched. (c) `messages.py:25 from ofscraper.db.operations_.media import (...)` re-entry — same root as (a) but a different consumer. Solving (b) likely requires making `screens.py:59 SCREENS["download"]["layout"]` lazy (factory pattern) rather than eagerly calling `get_download_group()` at module top, and auditing all `progress_utils.SCREENS[...]` callers (~14 sites). Solving (a)+(c) requires inverting the `db/operations_/media` ↔ `wrapper` ↔ `placeholder` ↔ `manager` graph — non-trivial. Architectural cleanup, multi-session work.
