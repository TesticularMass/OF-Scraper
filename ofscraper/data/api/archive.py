...
async def get_oldarchived(model_id, username):
    ...
    trace_log_raw("oldarchived", oldarchived)
    return oldarchived
...
async def scrape_archived_posts(
    ...
    try:
        while has_more:
            url = (
                of_env.getattr("archivedNextEP").format(
                    model_id, str(current_timestamp)
                )
        ...
        if all_ghosts_found:
            log.info(
                f"Worker finished. Flagging {len(all_ghosts_found)} ghost archived posts."
            )
...
async def get_after(model_id, username):
    ...
    # 1. Filter out what we already have, what is dead, OR what we don't want
    filtered_items = [
        x
        for x in curr
...
def time_log(username, after):
    log.info(
        f"Setting archived scan range for {username} from {arrow.get(after).format(of_env.getattr('API_DATE_FORMAT'))} to {arrow.get(settings.get_settings().before or arrow.now()).format((of_env.getattr('API_DATE_FORMAT')))}"
    )
